...
def admin_panel(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            assignment_instances = form.save() # the variable for the form instances
            
            # Send email to the support staff that a new assignment was done
            subject = "A new assignment issue {}".format(assignment_instances.issue.ticket_number) # obtain the ticket number of the issue assigned
            ...
            support_staff_email = assignment_instances.assigned_to.email # get the email of the support staff
            # Send the email now
            send_mail(subject, message, admin_email, [support_staff_email], fail_silently=False)
            
            # display success that the assigment was done
            messages.success(request, "Issue assigned to {} successfully".format(assignment_instances.assigned_to.username))
